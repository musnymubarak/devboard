...
# K8s Nodes Security Group — accepts traffic from ALB and within VPC
resource "aws_security_group" "k8s_nodes" {
  name        = "${var.project}-k8s-nodes-sg-${var.environment}"
  ...
  tags = { Name = "${var.project}-k8s-nodes-sg-${var.environment}" }
}

# RDS Security Group — only accepts traffic from K8s nodes
